...
<div class="input-group">
    <button onClick="men1(<?php echo $product->id; ?>)" type="button" class="btn btn-warning btn-flat" style="display: block; float: left; vertical-align: middle; width: 34px;">-</button>
		<input id="<?php echo $product->id; ?>" style="width: 50px; " type="number" min="0" max="<?php echo $q; ?>" value="1" step="any" class="form-control" required name="q" <?php if($product->fraction==1) { ?> onFocus="changervalor(<?php echo $product->id;?>)" <?php }else{ ?> onFocus="nochangervalor()"<?php } ?>)>
